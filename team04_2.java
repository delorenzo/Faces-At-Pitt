			...
			if(currentUserId == -1){
				...
				if(userInput == 1){//login task
					...
					try{
						...
						query = "SELECT userID FROM profile WHERE email=\'"+email+"\' AND password=\'"+password+"\'";
					...
					if(userExists){
						...
						try{
							statement = connection.createStatement();
							query = "SELECT lastLogin FROM profile WHERE userID="+userId;
					...
					if(userInputValid){
						...
						try{
							statement = connection.createStatement();
							query = "INSERT INTO profile VALUES("+(maxUserId+1)+", \'"+name+"\', \'"+email+"\', \'"+password+"\', TO_DATE(\'"+birthday+"\', \'yyyy-mm-dd\'), \'"+imageUrl+"\', \'"+selfDescription+"\', CURRENT_DATE)";
						...
						try{
							statement = connection.createStatement();
							query = "SELECT lastLogin FROM profile WHERE userID="+(maxUserId+1);
				...
				if(userInput == 3){//Send Message task
					...
					if(messageTypeInput.compareTo("a")==0){//send to user
						...
						if(toUserId != -1){
							...
							try{
								statement = connection.createStatement();
								query = "INSERT INTO Messages VALUES("+(maxMsgId+1)+", "+currentUserId+", \'"+message+"\', "+toUserId+", NULL, CURRENT_DATE)";
					...
					try {
						statement = connection.createStatement();
						query = "SELECT userID, name FROM profile where userID not ="+currentUserId+"ORDER BY userID DESC";
					...
					for (int i = 0; i < friendsoffriends.size(); i++) { //note this is the same as the size of friends
						for (int j = 0; j < friendsoffriends.get(i).size()-1; i++) {
					...
					*/
					...
					Scanner sc = new Scanner(System.in); //start a new scanner so we don't foul up the old one
					sc.useDelimiter("\\n"); //delimiter
					while (sc.hasNext()) {
						...
						try {
							query = "SELECT * FROM profile WHERE userID = ? or name = ? or email = ? or date_of_birth = ? or aboutme = ?";
							preparedStatement = connection.prepareStatement(query);
							...
							while (resultSet.next()) {
								results.add("UserID:  " + resultSet.getInt(1) + "/nName:  " + resultSet.getString(2) +
										"/nEmail:  " + resultSet.getString(3) + "/nDate of birth:  " + resultSet.getDate(4) +
										"/nPicture url:  " + resultSet.getString(5) + "/nAbout me:  " + resultSet.getString(6) +
										"/nLast login:  " + resultSet.getDate(7));
							}
						} catch(Exception Ex){
						...
						if (input.equals("ALL") || input.equals("all")) {
							...
							try {
								...
								while (resultSet.next()) {
									...
									s.setString(4, resultSet.getString(3));
									s.executeUpdate();
								}
