...
create table friends (
	...
	constraint fk_friends_user1 foreign key (userID1) references profile(userID),
	constraint fk_friends_user2 foreign key (userID2) references profile(userID)
);
...
create table messageRecipient (
	...
	constraint fk_messageRecipient_msgID foreign key (msgID) references messages (msgID),
...
create or replace trigger drop_acct
	...
	begin
		...
		delete from messages
			where fromID =:old.userID and ToUserID is null;
		delete from messages
			where ToUserID =:old.userID and fromID is null;
		update messages
			set fromID = null
